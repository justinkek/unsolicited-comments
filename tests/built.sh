...
#
# It builds once per run of the suite: run-tests sets UNSOLICITED_COMMENTS_BUILT,
# and a test run on its own builds its own.

if [ -n "${UNSOLICITED_COMMENTS_BUILT-}" ] && [ -d "$UNSOLICITED_COMMENTS_BUILT/claude/hooks" ]; then
  BUILT_HOOKS="$UNSOLICITED_COMMENTS_BUILT/claude/hooks"
else
  BUILT_ROOT="$(mktemp -d)"
