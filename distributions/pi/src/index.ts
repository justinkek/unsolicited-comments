...
const registered: Record<string, string[]> = {"SessionStart":["load-rules.sh","mark-session-started.sh"],"PreToolUse":["refuse-a-comment.sh"],"UserPromptSubmit":["note-a-new-version.sh","print-session-start-if-missed.sh","replay-notes.sh"]};

const hooks = join(dirname(fileURLToPath(import.meta.url)), "..", "hooks");
const notes = mkdtempSync(join(tmpdir(), "unsolicited-comments-notes-"));
const session = "pi";
let started = false;
...
function spawnHook(script: string, payload: Record<string, unknown>): string {
	try {
		return execFileSync("bash", [join(hooks, script)], {
			...
			env: { ...process.env, UNSOLICITED_COMMENTS_STOP_NOTE_DIRECTORY: notes },
			encoding: "utf8",
		}).trim();
	} catch {
		return "";
...
function decided(event: any, ctx: any): { decision: string; reason: string } | undefined {
	...
	try {
		...
		return { decision: "deny", reason: "unsolicited-comments could not read what its hooks decided." };
	}
...
export default function (pi: ExtensionAPI) {
	pi.on("before_agent_start", async () => {
		...
		return content ? { message: { customType: "unsolicited-comments", content, display: true } } : undefined;
	});
	...
	pi.on("tool_call", async (event: any, ctx: any) => {
		...
		if (answer.decision === "ask" && ctx?.hasUI && ctx?.ui?.confirm) {
			if (await ctx.ui.confirm("unsolicited-comments", answer.reason)) return undefined;
		}
