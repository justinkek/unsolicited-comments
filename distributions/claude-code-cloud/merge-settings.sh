...
stated=/opt/unsolicited-comments/distributions/claude-code-cloud

if ! command -v jq >/dev/null 2>&1; then
  printf 'unsolicited-comments: no jq to merge with, so %s was left as it was and no hooks are registered\n' \
    "$settings" >&2
  exit 1
fi
...
run() {
  ...
  rm -f "$written"
  printf 'unsolicited-comments: could not write %s, so it was left as it was\n' "$settings" >&2
  return 1
}
...
if ! jq --exit-status 'type == "object"' "$settings" >/dev/null 2>&1; then
  ...
  printf 'unsolicited-comments: %s did not read as an object, so it was kept as %s.unreadable and written fresh\n' \
    "$settings" "$settings" >&2
  fresh
  exit
fi
...
def mine: (.command? // "") as $command
  | ($command | contains($checkout + "/")) or ($command | contains("/unsolicited-comments/"));
