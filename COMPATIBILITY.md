# Where unsolicited-comments can be installed
...
- **Claude Cowork** - Every conversation runs on Anthropic's servers, whichever app starts it.
- **Claude Code, in the cloud** - The container is rebuilt for every session, so a setting kept goes in an environment variable.
